...
class GoalGaussianDiffusion_SD(nn.Module):
    def __init__(
        self,
        model,
        *,
    ...
    def p_sample(self, x, t: int, x_cond, task_embed, guidance_weight=0):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x, batched_times, x_cond, task_embed, clip_denoised = False, guidance_weight=guidance_weight)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        # self.guidance = StableDiffusion(torch.device('cuda'))#self.device)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    goal_embed = torch.zeros([1,3,512]).float().to(device)
